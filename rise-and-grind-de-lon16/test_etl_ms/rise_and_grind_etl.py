...
def normalise_data(rows, initial_locations, initial_products): #initial_locations):
    ...
    for row in rows:
        ...
        for item in items:  # to avoid duplicates (if there are any)
            ...
            product_flavour_key = product_flavour if product_flavour else ""
            product_size_key = product_size if product_size else ""

            product_key = (product_name, product_flavour_key, product_size_key)

            LOGGER.debug(f"Product Details: name='{product_name}', flavour='{product_flavour_key}', size='{product_size_key}'")
            LOGGER.debug(f"product_key='{product_key}'")
            ...
            if product_key not in product_lookup:
                ...
                LOGGER.debug(f"Duplicate product found! product_id='{product_id}'")
